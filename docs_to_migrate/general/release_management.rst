...
* A wizard has been implemented that allows the user to define the location of the output artifacts belonging to an instance. All output artifacts can be moved simultaneously into a dedicated folder of choice, either during instance creation or afterwards - see :ref:`instance_creation`.
...
* Spark configuration can be changed from the UI by creating custom runtimes, see :ref:`spark_config`.
...
* When an :term:`Instance` is shared amongst the users, the associated runtime is shared as well with the users (previously, the runtime had to be shared separately).
...
* More documentation has been provided in which situations users can or cannot download files from the UI when clicking on the URL - see :ref:`download_artifact_version_file`.
* More explicit documentation on expected behaviour in context of access tokens and refresh tokens - see :ref:`token_info`.
...
* Some validation checks on uniqueness of artifact version names have been relaxed - see :ref:`register_artifact` notes.
* While choosing your inputs during 'open code', the search when choosing an artifact version has been fixed - see :ref:`spec_notebook`.
...
* The log format sent by the backend has been changed to have more clear logs - see :ref:`view_logs`.
...
* To enable users to take further ownership of their data (e.g. clean-up, satisfy with GPDR regulation, save disk space etc.), we allow the deletion of artifact versions - see :ref:`deletion_artifact_version`. Having the capability of deleting only versions, and not the entire artifact, means that existing instances that use some particular input artifacts can continue to be used (instead of having to recreate instances after deleting an entire artifact).
...
* We enable the users to limit the number of allowed actions on objects that they share with other users by having the capability to reserve an object.
  The main use case for this functionality is to keep better/more restrictive control on shared objects - see :ref:`reserve_object`, for example:
...
* The user has the possibility to toggle between the default object card view and a list view of objects, rendering more objects on a single page in a paginated, tabular format - see :ref:`list_view`.
* We extended the attributes that the objects can be sorted by. This should allow the user to navigate more easily amongst the objects. See :ref:`sorting_attributes`.
...
* During the registration of an artifact version, the user has the option to mark a given version as being a correction of another - deemed erroneous - version. This 'correction attribute' can then be used to read only 'corrected' artifact versions. See :ref:`Correcting an existing artifact version`. This to replace the sample level versioning that is removed by getting rid of Hudi.
* During instance run, the user has the possibility to select tags that can be interpreted by our Python API only to provide those artifact versions having all the tags associated, see :ref:`spec_notebook`.
...
* Users can tag objects (:term:`Artifact` (including artifact versions), :term:`Instance` (including sessions)), that can be used to filter over when searching or retrieving a given object. Tags can be used both from within the UI as well as in a programmatically manner. See :ref:`tag_procedure`.
...
* The user journey has been simplified/enhanced on the following points:
    ...
    * When creating a :term:`Specification`, the user should no longer select an 'accepted model type' but can immediately start defining inputs.
    * When an :term:`Instance` is created, this instance should no longer be registered. It can immediately be executed ('run').
