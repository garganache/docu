...
metadata, ... see :ref:`create_spec`) from the current session is retrieved from the registry and exposed via this
...
docstrings documentation by using :code:`help()`.
...
* :code:`session` - Starting point of the library.
* :code:`session.io` - Interface for accessing input and output artifacts, parameters, and more.
* :code:`session.manager` - Interface to manage the current session such as tagging objects, and more.
* :code:`session.direct` - Direct access to the registry API.

*********
Providers
*********

We currently support the following :term:`providers`:
...
For working examples on how to actually use these providers, see :ref:`library_io`.
