---
sidebar_label: "Python I/O Operations"
---

# IO

As a prerequisite, the relevant modules should be loaded (see [Overview of modules](python-library-modules.md)).

## Parameters

For parameters defined when creating a specification (see [Creating a specification](../user-guide/specification-creation.md#create-a-specification)), their attributes can be retrieved inside
...
The `parameter.name` attribute is the alias defined by the user during the creation of a specification.
...
The --extra-index-url is optional and above is inserted just as an example. If using it make sure that you keep the USERNAME and PASSWORD in the [Resource](../concepts/glossary.md#resource) object.
...
* The artifact version that was chosen (see [Retrieving a specific artifact version](#retrieving-a-specific-artifact-version)).
* A list of all artifact versions previous to the chosen one (see [Retrieving all previous artifact versions](#retrieving-all-previous-artifact-versions)).
...
The [Artifact Version](../concepts/glossary.md#artifact-version) that is chosen from the UI can be read as follows:
...
If the user has chosen a tag in the UI for the filter (see [Specification notebook](../user-guide/specification-creation.md#2-open-a-specifications-notebook)), the following applies:
...
The tags with serve the purpose of an additional filter over the [Artifact Version](../concepts/glossary.md#artifact-version)(s) to be read.
...
* [Correcting an existing artifact version](#artifact-corrections)
* [Tag procedures](python-tag-management.md)
* [Specification notebook](../user-guide/specification-creation.md#2-open-a-specifications-notebook)

:::

Sometimes, it can be useful to have an aggregated view in which all data from all artifact versions is gathered.
Beginning from the [Artifact Version](../concepts/glossary.md#artifact-version) that is chosen from the UI, all previous versions can be retrieved as
follows:
...
* If the user selects filter tags, only artifact versions having those tags associated shall be returned by the previous_versions.
  ...
  The tags during 'open code' - see [Specification notebook](../user-guide/specification-creation.md#2-open-a-specifications-notebook) - serve the purpose of an additional filter over the
  [Artifact Version](../concepts/glossary.md#artifact-version)(s) to be read.
* In case of multiple tags selected, only artifact versions having all of them (AND tag1 AND tag2 etc.) shall be
  returned.
...
The idea of the library is to provide the user easy access to the file. From that point onwards, it's up to the user to
...
So, instead of using the `.open()` method; the `.dataframe()` should be used where the format of the file to
...
The code to read from HDFS is virtually the same as the local file system (see [From the local file system](#from-the-local-file-system)).
...
See [Writing to HDFS with Spark](#with-spark-2).
...
correction of a previous version (see [Correcting an existing artifact version](../user-guide/artifact-management.md#correcting-an-existing-artifact-version)).
...
![Artifact correction result](/img/app-a/correction_of_python.png)

## Create a Spark session

:::note

Files from either the local filesystem or HDFS can already be read as Spark dataframes, see [With Spark (local filesystem)](#with-spark) and [With Spark (HDFS)](#with-spark-1). The code below is considered useful only in case user wants to create a Spark dataframe from scratch.
