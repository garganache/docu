---
sidebar_label: "Python Session API"
---
...
Once the session is initialised (see [Overview of modules](python-library-modules.md)), the user can retrieve the session's metadata via the following
methods.
...
![Session card message](/img/app-a/session_card_message.png)
...
Following values can be provided: `TRACE`, `DEBUG`, `INFO`, `SUCCESS`,
`WARNING` (default value), `ERROR`, `CRITICAL`.
...
It's possible to send custom metrics to prometheus via `session.manager.metric`. Session id should be sent as a
...
Counting events? --> Counter<br/>
Current value that fluctuates? --> Gauge<br/>
Need distribution across multiple services? --> Histogram<br/>
...
`session.manager.metric` is available for sending metrics to prometheus.

- `metric`: Name of the metric you want to track (e.g., 'yfp_session_warning').
- `primitive`: Type of metric - 'gauge' for values that go up and down, 'counter' for values that only increase.
- `value`: The measurement value to record.
- `docs`: Description of what this metric measures (optional but recommended).
- `group_keys`: Categories to organize this metric by (optional). Useful for grouping related metrics together in
the monitoring dashboard
- `kwargs`: Additional tags to label this metric, like session_id or instance_id. These help filter and analyze
metrics later.
...
`session.manager.metric_exception` is available for sending exceptions to prometheus.
